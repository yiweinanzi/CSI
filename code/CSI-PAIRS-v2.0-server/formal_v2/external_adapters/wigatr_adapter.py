...
def run_adapter(args) -> dict:
    ...
    if provisional_root is None:
        raise RuntimeError("Wi-GATr requires output and upstream roots")
...
def _resolve_run_roots(args, dataset, config, output):
    ...
    if run_root is not None:
        if output_run_root is not None or upstream_root is not None:
            raise RuntimeError("Wi-GATr roots cannot mix local and explicit modes")
        local_root = Path(run_root).resolve()
        if (local_root / "migration").exists():
            raise RuntimeError("Wi-GATr migration requires explicit output and upstream roots")
    ...
    if output != output_run and output_run not in output.parents:
        raise RuntimeError("Wi-GATr output escapes the output run root")
    ...
    if authenticated.run_root != output_run or expected_upstream != upstream:
        raise RuntimeError("Wi-GATr output/upstream root binding mismatch")
